...
When using `SqsTemplate` with `TemplateAcknowledgementMode#MANUAL` or `TemplateAcknowledgementMode#ON_SUCCESS` and `SendBatchFailureHandlingStrategy#THROW`, failures in batch operations can occur. In such cases, `Spring Cloud AWS` provides `SqsAcknowledgementException` to handle these failures.

==== SqsAcknowledgementException

`SqsAcknowledgementException` is thrown when a batch operation fails, and it contains information about both successful and failed acknowledgements.

```java
public class SqsAcknowledgementException extends RuntimeException {

    private final Collection<Message<T>> successfullyAcknowledgedMessages;
    private final Collection<Message<T>> failedAcknowledgementMessages;
...
When a partial failure occurs, `SqsAcknowledgementException` will contain:

- `successfullyAcknowledgedMessages`: Messages that were successfully acknowledged.
- `failedAcknowledgementMessages`: Messages that failed to be acknowledged.
- `queue`: The queue URL where the operation was performed.
...
private AcknowledgementResultCallback<Object> getAcknowledgementResultCallback() {
    return new AcknowledgementResultCallback<Object>() {
        ...
        public void onSuccess(Collection<Message<Object>> messages) {
            // Handle successful acknowledgements
        }

        @Override
        public void onFailure(Collection<Message<Object>> messages, Throwable t) {
            if (t instanceof SqsAcknowledgementException sqsAckEx) {
                Collection<Message<Object>> successfullyAcknowledgedMessages = sqsAckEx.getSuccessfullyAcknowledgedMessages();
                Collection<Message<Object>> failedAcknowledgementMessages = sqsAckEx.getFailedAcknowledgementMessages();
                String queue = sqsAckEx.getQueue();
                
                // Handle partial failures
                // For example, retry failed messages or log the error
            }
        }
    };
}
